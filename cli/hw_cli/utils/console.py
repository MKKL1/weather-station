...
def should_use_color(file=None) -> bool:
    if file is None:
        file = sys.stderr
...
def print_info(msg: str, file=None):
    if file is None:
        file = sys.stderr
...
def print_success(msg: str, file=None):
    if file is None:
        file = sys.stderr
    if should_use_color(file):
...
def print_table_header(columns: List[Tuple[str, int]], file=None):
    if file is None:
        file = sys.stdout
...
def print_table_row(values: List[str], widths: List[int], file=None):
    if file is None:
        file = sys.stdout
